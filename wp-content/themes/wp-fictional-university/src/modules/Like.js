...
class Like {
    constructor() {
        if (document.querySelector(".like-box")) {
            axios.defaults.headers.common["X-WP-Nonce"] = wpFictionalUniversityData.nonce;
            this.events();
        }
    }
    ...
    async createLike(currentLikeBox) {
        const url = wpFictionalUniversityData.root_url + "/wp-json/wp_fictional_university/v1/manageLike";
        const newLike = {
            "professorId": currentLikeBox.getAttribute("data-professor")
        }

        try {
            const response = await axios.post(url, newLike);
    ...
    async deleteLike(currentLikeBox) {
        try {
            const response = await axios({
                url: wpFictionalUniversityData.root_url +  "/wp-json/wp_fictional_university/v1/manageLike",
                method: 'delete',
